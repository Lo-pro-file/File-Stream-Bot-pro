...
if alredy == "n":
    ...
    while not re.match("[0-9]+", api_id):
        print(f"{Fore.RED}Invalid API ID, please try again...{Style.RESET_ALL}")
        api_id = input("Enter your API ID: ")
    api_hash = input("Enter your API HASH: ")
    while not re.match("[A-Za-z0-9]+", api_hash):
        print(f"{Fore.RED}Invalid API HASH, please try again...{Style.RESET_ALL}")
        api_hash = input("Enter your API HASH: ")
    bot_token = input("Enter your bot token: ")
    while not re.match("[0-9]+:[A-Za-z0-9_-]+", bot_token):
        print(f"{Fore.RED}Invalid bot token, please try again...{Style.RESET_ALL}")
        bot_token = input("Enter your bot token: ")

    fqdn = input("Enter your server id:")
    while not re.match("[A-Za-z0-9]+", fqdn):
        print(f"{Fore.RED}Invalid server id, please try again...{Style.RESET_ALL}")
        fqdn = input("Enter your server id:")
    username = input("Enter your username id, you can get it from @MissRose_bot:")
    while not re.match("[0-9]+", username):
        print(f"{Fore.RED}Invalid username id, please try again...{Style.RESET_ALL}")
        username = input("Enter your username id, you can get it from @MissRose_bot:")
    ...
    while not re.match("^mongodb(?:\+srv)?:\/\/(?:\S+(?::\S*)?@)?(?:(?:(?!-)[A-Za-z0-9-]{1,63}(?<!-)\.)+(?:[A-Za-z]{2,}|xn--[A-Za-z0-9]{2,}))(:[0-9]{1,5})?(?:\/(?:[^\s/?#]+(?:\?[^#\s]*)?)?)?$", database):
        print(f"{Fore.RED}Invalid mongodb url, please try again...{Style.RESET_ALL}")
        database = input("Enter your mongodb url:")
    ...
    if input == "y":
        ...
        if banned_channels != "":
            ...
            print(f"{Fore.YELLOW}No data provided, skipping...{Style.RESET_ALL}")
